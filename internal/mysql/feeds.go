...
func (r Repo) InsertEntries(ctx context.Context, entries []seymour.FeedEntry) error {
	...
	// MySQL has no ON CONFLICT clause; INSERT IGNORE skips rows that would
	// violate the guid unique index, same effect as sqlite's
	// ON CONFLICT(guid) DO NOTHING.
	const q = `INSERT IGNORE INTO feed_entries (id, feed_id, title, description, guid, link, publish_time)
